...
| 能力 | 命令 / 行为 |
|------|-------------|
| 上电自动 | 未配对：20s 内多次 `pair`；已配对：异步重连（优先内存 `last_channel`），失败则 20s 配对窗 |
| 空闲休眠 | 已连接 **60s 无按键** → **RADIO + USB** 休眠，主循环等 GPIOTE |
| 按键唤醒 | `sw0` GPIOTE：唤醒 USB+RADIO → 先打 `last_channel` → 立刻发键；未配对忽略 |
| 配对 | `pair` → 写 NVS（含 aes_counter） |
| 按键 | `key a` 异步入队；`aes_counter` 只改 RAM，**进休眠才刷 NVS** |
| 主动休眠 | `sleep` → 刷 NVS、关 RADIO、关 USB |
| 休眠重连 | 物理键 / `wake` / `key` → 异步恢复 |
...
1. **未配对**：上电后约 **20s** 内多次尝试 `pair`。超时 → `sleeping`（RADIO+USB 关）
2. **已配对**：异步重连，**优先内存中上次信道**（不强制信道 5）；成功 → 60s 无按键休眠；失败 → 20s 配对窗
3. **已连接**：60s 无按键 → 休眠（刷 `aes_counter` 到 NVS，关 RADIO+USB）
4. **物理按键（P0.02/D19，GPIOTE）**：
   - 未配对 → 忽略
   - 休眠/未连接且已配对 → 恢复 USB → 先 `last_channel` 重连 → 立刻发 `a`
   - 配对窗内有凭证时按键：重连成功则停止配对并发键；失败则继续配对窗
5. **休眠时 COM 口可能断开**；唤醒后需重新打开串口助手
...
`sleep` → `status` 不可用（USB 已关）；按键 GPIOTE 可唤醒。

### 4. 休眠重连

物理键或唤醒后重开串口发 `wake` / `key a`。重连优先上次信道，异步步进不堵死主循环。

### 5. 断电重连

已配对后复位 → 异步重连；失败进 20s 配对窗。`last_channel` 仅在 RAM（冷启动从默认表头开始，成功后会更新）。
